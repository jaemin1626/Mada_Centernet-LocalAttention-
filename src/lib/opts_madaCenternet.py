...
class opts(object):
  def __init__(self):
    ...
    #self.parser.add_argument('task', default='ctdet', # 디폴트
    #                         help='ctdet | ddd | multi_pose | exdet | ctdet_multi')
    self.parser.add_argument('--dataset', default='pest_coco_2',
                             help='coco | pest_coco | pest_coco_2 | kitti | coco_hp | pascal')
    self.parser.add_argument('--exp_id', default='coco_hg_multi_scale') # 수정
    self.parser.add_argument('--test', action='store_true')
    self.parser.add_argument('--debug', type=int, default= 2, # 수정
    ...
    self.parser.add_argument('--demo', default='', 
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default='/home/ai001/MadaCenternet_(LocalAttention)/save_dir/MadaCenternet_(LocalAttention)/model_14.pth', # 수정 '../models/ExtremeNet_500000.pth'
    ...
    # system
    # self.parser.add_argument('--gpus', default='0,1', # 수정
    #                         help='-1 for CPU, use comma for multiple gpus')
    self.parser.add_argument('--num_workers', type=int, default=1, # 수정
    ...
    self.parser.add_argument('--arch', default='mada_centernet_localAttention',#hourglass_multi_scale_with_multiscale_dcn',  # 수정
                             help='model architecture. Currently tested'
                                  'hourglass | mada_centernet_localAttention | mada_centernet_dualAttention | centernet_localAttention')
    self.parser.add_argument('--head_conv', type=int, default=-1,
                             help='conv layer channels for output head'
                                  '0 for no conv layer'
                                  '-1 for default setting: '
                                  '64 for resnets and 256 for dla.')
    self.parser.add_argument('--down_ratio', type=int, default= 2, ## 수정 2
    ...
    # train
    self.parser.add_argument('--lr', type=float, default=0.0001, # 수정 
                             help='learning rate for batch size 32.')
    self.parser.add_argument('--lr_step', type=str, default=None, # 수정
                             help='drop learning rate by 10.')
    self.parser.add_argument('--num_epochs', type=int, default=100, # 수정
                             help='total training epochs.')
    self.parser.add_argument('--batch_size', type=int, default=8, # 수정
                             help='batch size')
    self.parser.add_argument('--master_batch_size', type=int, default=1, # 수정
    ...
    self.parser.add_argument('--K', type=int, default=250,
    ...
    self.parser.add_argument('--keep_res', default=False,
                             help='keep the original resolution'
                                  ' during validation.')

    # dataset, 항상 데이터 크롭이 들어가기에 shift, scale, rotate 적용 X. flip은 값을 주면 적용된다.
    self.parser.add_argument('--not_rand_crop', default=True,
    ...
    self.parser.add_argument('--no_color_aug', default = True, # 수정
  ...
  def parse(self, args=''):
    ...
    # opt.data_dir = '../data'
    # opt.exp_dir = '../exp'
    # opt.save_dir = '../exp/coco_hg_multiscale'
    opt.debug_dir = '/home/ai001/MadaCenternet_(LocalAttention)/debug'
    opt.root_dir = os.path.join(os.path.dirname(__file__), '..', '..')
    opt.data_dir = "/home/ai001/MadaCenternet_(LocalAttention)/data_2023"
    opt.exp_dir = "/home/ai001/MadaCenternet_(LocalAttention)/exp"
    opt.save_dir = "/home/ai001/MadaCenternet_(LocalAttention)/save_dir/2023_data/MadaCenternet(LocalAttention)_2023"
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    elif opt.task == 'ctdet_multi' or opt.task == 'ctdet_multi_kdmg':
        # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    #   opt.heads.update({ 'hr_hm' : opt.num_classes,
    #                 'hr_wh' : 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
    #                 'hr_reg' : 2})
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 1, 
                ...
                'dataset': 'pest_coco'},
      'ctdet_multi': {'default_resolution': [512, 512], 'num_classes': 1, # 수정
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'pest_coco'}, # 수정
