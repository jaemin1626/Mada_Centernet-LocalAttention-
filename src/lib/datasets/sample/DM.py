...
class DMDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    trans_output_ori = get_affine_transform(c, s, 0, [input_w, input_h])
    ...
    for k in range(num_objs):
      ...
      bbox_hr[:2] = affine_transform(bbox_hr[:2], trans_output_ori)
      bbox_hr[2:] = affine_transform(bbox_hr[2:], trans_output_ori)
      bbox_hr[[0, 2]] = np.clip(bbox_hr[[0, 2]], 0, input_w - 1)
      bbox_hr[[1, 3]] = np.clip(bbox_hr[[1, 3]], 0, input_h - 1)
    ...
    ret = {'input': inp, 'hm': lr_density, 'points' : points, 'file_name' : file_name}
