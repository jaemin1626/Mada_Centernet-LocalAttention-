...
def gaussian_filter_density(points, radius, img_size= None, adaptive_mode=False, fixed_value=15, fixed_values=None):
    ...
    for idx, (p, radius) in enumerate(zip(points, radius)):
        ...
        p[1], p[0] = min(h-1, p[1]), min(w-1, p[0])
        if num_gt > 1:
            if adaptive_mode == 1:
...
class check_DMDataset_MultiScale(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    for k in range(num_objs):
      ...
      if h > 0 and w > 0:
        ...
        #############################################################
        #############################################################
        lr_points.append(ct_int)
        lr_radius_list.append(radius)
        
        hr_points.append(hr_ct_int)
        hr_radius_list.append(hr_radius)
    ...
    lr_density = gaussian_filter_density(lr_points, lr_radius_list, img_size = (output_h, output_w), adaptive_mode=False)
    hr_density = gaussian_filter_density(hr_points, hr_radius_list, img_size = (hr_output_h, hr_output_w), adaptive_mode=False)
    
  

    # plt.imshow(hr_density, cmap=CM.jet)
    # plt.ioff()
    # plt.savefig('../exp/proposal_method/GroundTruth/densitymap_estimate/first_dataset/image_616_hr'+ '.jpg', dpi=100)
    ret = {'input': inp, 'hm': lr_density, 'hr_hm' : hr_density, 'file_name' : file_name}
