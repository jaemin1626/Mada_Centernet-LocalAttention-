...
class CTDetDataset_MultiScale(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    c_hr = np.array([img.shape[1]/2., img.shape[0]/2.], dtype=np.float32)
    ...
    ret = {'input': inp, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh,
           'hr_hm' : hr_hm, 'hr_reg_mask': hr_reg_mask, 'hr_ind':hr_ind, 'hr_wh':hr_wh, 'file_name' : file_name}
