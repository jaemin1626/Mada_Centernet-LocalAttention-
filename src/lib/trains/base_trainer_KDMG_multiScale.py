...
class ModelWithLoss(torch.nn.Module):
  ...
  def forward(self, batch):
    ...
    generated_heatmap = self.model2(batch['points'], batch['input'])
    loss, loss_stats = self.loss(outputs, batch, generated_heatmap)
...
class BaseTrainer(object):
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      for k in batch:
        if k != 'meta' and k != 'file_name' and k != 'points': # 수정
