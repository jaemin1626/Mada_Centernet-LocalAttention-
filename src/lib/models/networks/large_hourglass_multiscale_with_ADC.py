...
class KBDG(nn.Module):
    def __init__(self):
        super().__init__()


class Multiscale_ADC(nn.Module):
    def __init__(self, stack, dims = [256, 128, 64]):
        super().__init__()
        self.pre = nn.Sequential(
            convolution(3,3,256,2),
            residual(3,256,256,2)
        )
        self.msdcn_layers = nn.ModuleList([
                        Multiscale_DCN(dims[ind],dims[ind+1]) for ind in range(stack)
                        ])
        self.hm = nn.Sequential(
                        convolution(3, 64, 128, with_bn=False),
                        nn.Conv2d(128, 1, (1, 1))
                    )
    ...
    def forward(self, x):
        ...
        output = []
        for i, layer in enumerate(self.msdcn_layers):
            result.append(layer(result[i]))

        for layer in [self.hm, self.wh, self.off]:
            output.append(layer(result[-1]))
...
class ADC(nn.Module):
    def __init__(self, heads):
        super().__init__()
        self.kps  = HourglassNet(heads, num_stacks=1)
        self.msadc = Multiscale_ADC(2)
        self.sig = nn.Sigmoid()
        self.up1 = nn.ConvTranspose2d(1, 1, 3, stride=2, padding=1)
        self.up2 = nn.ConvTranspose2d(1, 1, 3, stride=2, padding=1)
    
    def forward(self,x):
        #ggt = self.kbdg(x)
        outs = self.kps(x)
        out_hm = outs[0]['hm']
        up1 = self.up1(out_hm, output_size=torch.Size([256, 256]))
        up2 = self.up2(up1, output_size=torch.Size([512, 512]))
        mask = self.sig(up2)
        #custom_imshow(mask)
        masked_x = x.mul(mask)
        adc_hm,adc_wh,adc_off = self.msadc(masked_x)
        outputs = [{'hm':adc_hm,'wh':adc_wh,'reg':adc_off}]
        return outputs

def get_large_hourglass_multi_scale_with_ADC(num_layers, heads, head_conv, num_stack = 2):
    if num_stack:
        pass
    else:
        num_stack = 2
