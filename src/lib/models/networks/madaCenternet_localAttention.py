    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            if ind == 0:
                ...
                kp_, cnv_  = self.kps[ind], self.cnvs[ind]
                kp = kp_(inter_1, feature_maps, dwn_hms) # hourglass 모듈
                cnv = cnv_(kp) # hourglass 모듈
            ...
            if ind < self.nstack - 1:
                ...
                # bound_1 = outs[0]['wh'][0][0].unsqueeze(0)
                # bound_2 = outs[0]['wh'][0][1].unsqueeze(0)
                
                # bound_1_mean = bound_1.mean()
                # bound_1_std = bound_1.std()
                # bound_1 = (bound_1- bound_1_mean) / bound_1_std
                               
                # bound_2_mean = bound_2.mean()
                # bound_2_std = bound_2.std()
                # bound_2 = (bound_2 - bound_2_mean) / bound_2_std
            
                ## offset map 정규화  ##                 
                # offset_1 = outs[0]['reg'][0][0].unsqueeze(0)
                # offset_2 = outs[0]['reg'][0][1].unsqueeze(0)
                
                # offset_1_mean = offset_1.mean()
                # offset_std = offset_1.std()
                # offset_1 = (offset_1- offset_1_mean) / offset_std
                
                # offset_2_mean = offset_2.mean()
                # offset_2_std = offset_2.std()
                # offset_2 = (offset_2- offset_2_mean) / offset_2_std
                
                # folder_path = '/home/ai001/MadaCenternet_(LocalAttention)/save_dir/MadaCenternet_(LocalAttention)/hm_image/'
                # file_list = os.listdir(folder_path)
                # file_count = len(file_list)
                # save_image(hm_1, '/home/ai001/MadaCenternet_(LocalAttention)/save_dir/MadaCenternet_(LocalAttention)/hm_image/hm_{}.png'.format(file_count))
                # save_image(bound_1,'/home/ai001/MadaCenternet_(LocalAttention)/save_dir/MadaCenternet_(LocalAttention)/bound1_image/bound1_{}.png'.format(file_count))
...
def get_madaCenternet_localAttention(num_layers, heads, head_conv, num_stack = 2):
    if num_stack:
        pass
    else:
        num_stack = 2
