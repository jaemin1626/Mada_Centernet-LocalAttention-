...
from models.utils import _sigmoid_no_clamp
...
class kp_module(nn.Module):
    ...
    def forward(self, x, feature_maps=None):
        if feature_maps is None:
            ...
            low2 = self.low2(low1)
            if len(low2) == 2 and low2[0].ndim == 4: 
...
class KBDG(nn.Module):
    ...
    ## downsample => 입력과 출력의 감소 비율
    def __init__(self, downsample=8, s=5, sigmoid=False, small=False):
        super(KBDG, self).__init__()
        ...
        for j, idx in enumerate(points):
            ...
            for i in range(n):
                if idx[i,1] == idx[i,0] == -1:
                    ...
                    # conv and sum
                    k = kernels[j,:,min(kernels.shape[2]-1,int(idx[i,1]/16)),min(kernels.shape[3]-1,int(idx[i,0]/16))].view(1,1,self.s,self.s)
                    ...
                    k_shape = k.shape
                    k = nn.Softmax(2)(k.reshape(1, 1, -1))
                    k = k.reshape(k_shape)
    ...
    def forward(self, image, points = []):
        ...
        for ind in range(self.nstack):
            ...
            for head in self.heads:
                ...
                # print(layer)
                y = _sigmoid_no_clamp(layer(cnv)) # 히트맵, wh, offset에 대한 Loss를 구하기 위해 컨볼루션 연산 수행
                out[head] = y 
            ...
            if ind < self.nstack - 1:
                ...
                ##추가##
                sum_of_hm = torch.sum(outs[0]['hm'])
                sum_of_uphm = torch.sum(uphm)
                uphm = (uphm*sum_of_hm)/sum_of_uphm
                ##추가##
...
def get_large_hourglass_net_multi_dcn_with_multiscale_dcn_estimation(num_layers, heads, head_conv, num_stack = 2):
    if num_stack:
        pass
    else:
        num_stack = 2
