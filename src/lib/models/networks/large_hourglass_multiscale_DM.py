...
class exkp(nn.Module):
    def __init__(
        ...
        # 추가
        #self.tconv = transposedconvolution(k=2, inp_dim = 256, out_dim = 256, stride=2)

        # self.upsample = nn.UpsamplingBilinear2d(scale_factor=2)
        # self.upconv = convolution(k=1, inp_dim=256, out_dim=256, stride=1)
        ...
        #     nn.UpsamplingBilinear2d(scale_factor=2),
        #     convolution(k=1, inp_dim=2, out_dim=2, stride=1, with_bn=False),
        #     nn.Conv2d(2, 2, (1, 1))
        # )
        ...
        # self.conv = convolution(k=3, inp_dim=261, out_dim=256, stride=1)
        ########################################################################
    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            if ind == 0:
                ...
                kp  = kp_(inter) # hourglass 모듈
                cnv = cnv_(kp) # hourglass 모듈
            elif ind == 1:
                kp_, cnv_  = self.kps[ind], self.cnvs[ind]
                kp  = kp_(inter_1) # hourglass 모듈
                cnv = cnv_(kp) # hourglass 모듈
...
def get_large_hourglass_net_multi_DM(num_layers, heads, head_conv, num_stack = 2):
    if num_stack:
        pass
    else:
        num_stack = 2
