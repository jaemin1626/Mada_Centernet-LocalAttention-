...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'pest_coco': # 수정
      self.names = pest_class_name
    elif num_classes == 1 or dataset == 'pest_coco_2':
      self.names = pest_class_name
    elif num_classes == 1 or dataset == "crop_pest_coco":
      self.names = pest_class_name
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0,0,255), 3)
    # if show_txt:
  ...
  def save_all_imgs(self, path='./cache/debug/', image_path = "", count = None, prefix='', genID=False):
    if genID:
      try:
    ...
    for i, v in self.imgs.items():
      ...
      # if 'pred_hm' in i:
      #   cv2.imwrite(path + "/hm" +'/hm_{}_{}.png'.format(count,image_path[0]), v)

      if 'ctdet' in i: ## 수정
        cv2.imwrite(path + "/ctdet" +'/ctdet_{}_{}.png'.format(count,image_path[0]), v)
